...
import { editAwardCategorysAction } from '../../redux/slices/AwardCategories/AwardCategorySlices';
  ...
  const dispatch = useDispatch()
    ...
    const formik = useFormik({
        initialValues: {
            ...
            optForNotification: category?.optForNotification,
...
// add the created category to state 
    ...
    const { isawardCategoryUpdated } = awardCategories    
    //navigation and history
    const navigate = useNavigate()
    //dispatch
    useEffect(() => {
        if (isawardCategoryUpdated) navigate({pathname:'/all-awards'})
    }, [isawardCategoryUpdated, dispatch, navigate])


    // manage state of datepicker

    const [startDate, setStartDate]=useState(new Date(category?.startDate))
    const [endDate, setEndDate]=useState(new Date(category?.endDate))
